// ============================================================================
// UNIFIED LOOPIFY APP - Production-grade unified sustainability platform
// ============================================================================
...
// CLASSIFICATION RULES (SVM-style)
...
function classifyWaste(imgElement) {
  ...
  for (const [category, rules] of Object.entries(CLASSIFICATION_RULES)) {
    ...
    // Keyword matching (40%)
    const keywords = filename.toLowerCase().concat(' ');
    ...
    score = keywordScore + featureScore + colorScore;
...
function extractWasteFeatures(imgElement) {
  ...
  const hasNaturalColors = colors.brown > totalPixels * 0.1 || colors.green > totalPixels * 0.1;
...
function setupShelfLife() {
  document.getElementById('shelflife-form').addEventListener('submit', (e) => {
    ...
    const item = addFoodItem(name, date, storage);
    displayFoodItems();
...
function displayFoodItems() {
  ...
  if (AppState.foodItems.length === 0) {
    container.innerHTML = '<p class="text-muted text-sm">No items yet. Add something to see predictions.</p>';
    return;
  }
  ...
  const activeItems = AppState.foodItems.filter(item => {
    ...
    return (now - itemDate) / (1000 * 60 * 60 * 24) < 30; // Show items from last 30 days
  });
      ...
      <div class="card-premium">
        <div class="flex justify-between items-start mb-2">
          <div>
            ...
            <p class="text-xs text-muted">${item.storage}</p>
          </div>
          <div class="badge-sage">${item.daysRemaining} days</div>
        </div>
        <div style="border-top: 1px solid #333; padding-top: 8px; margin-top: 8px;">
          <p class="text-xs" style="color: ${riskColor}; font-weight: 500;">${riskLabel}</p>
        </div>
      </div>
    `;
  }).join('');
...
function displayReturnsLog() {
  ...
  log.innerHTML = recentReturns.map(ret => `
    <div style="padding: 12px 0; border-bottom: 1px solid #333;">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
          <div style="font-weight: 500; font-size: 14px;">${ret.itemId}</div>
          <div style="font-size: 12px; color: #999;">${ret.condition}</div>
        </div>
        <div style="text-align: right;">
          <div class="badge-sage">${ret.status}</div>
...
// ============================================================================
// MATERIALBANK - INDUSTRIAL MATCHING
// ============================================================================
...
function displayMaterialsTable() {
  ...
  tbody.innerHTML = AppState.materials.map(mat => `
    <tr>
      <td>${mat.name}</td>
      <td>${mat.category}</td>
      <td>${mat.quantity}</td>
      <td><span class="badge-sage">${mat.type}</span></td>
    </tr>
  `).join('');
}
...
function updateImpactDashboard() {
  // Update metrics
  // Sync with AR detection counter from localStorage (set by ar-education.html)
  let arImpact = {};
  try {
    arImpact = JSON.parse(localStorage.getItem('loopify-impact')) || {};
  } catch (e) { arImpact = {}; }
  // Use the higher value between AppState and AR for wasteClassified
  const totalWasteClassified = Math.max(AppState.impact.wasteClassified, arImpact.wasteClassified || 0);
  document.getElementById('metric-waste-diverted').textContent = totalWasteClassified;
  // Optionally, update AppState to keep in sync
  AppState.impact.wasteClassified = totalWasteClassified;
